...
export const WORKFLOW_STEPS = [
  ...
  { id: 2, key: "baseline", label: "02 Baseline", description: "Hypothesis & Prompting" },
...
export function Stepper({ currentStep, onSelectStep, completedSteps }: StepperProps) {
  return (
    <div className="border-b border-slate-800 bg-slate-900/50 py-3 backdrop-blur-sm overflow-x-auto">
            ...
            >
              <div className="flex h-7 w-7 items-center justify-center">
                {isCompleted ? (
                  ...
                  <div className="h-4 w-4 rounded-full bg-indigo-500 ring-4 ring-indigo-500/20" />
                ) : (
                  <Circle className="h-4 w-4 text-slate-600 group-hover:text-slate-400" />
                )}
              </div>
              <div className="hidden sm:block">
                <div className={`text-xs font-bold ${isCurrent ? "text-indigo-300" : ""}`}>
                  {step.label}
                </div>
                <div className="text-[10px] text-slate-500 leading-none">{step.description}</div>
              </div>
            </button>
          );
        })}
