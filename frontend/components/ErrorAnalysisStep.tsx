        ...
        <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm space-y-4">
          <div className="border-b border-slate-100 pb-3 flex items-center justify-between">
            <div className="flex items-center space-x-2">
              ...
              <h3 className="text-sm font-bold text-slate-900">Top Misclassification Pairs</h3>
            </div>
            <span className="text-xs text-slate-500 font-medium">{errorAnalysis.total_errors} Total Errors</span>
          </div>

          <div className="space-y-3">
            {errorAnalysis.top_confusions.map((item, idx) => (
              <div key={idx} className="rounded-lg bg-slate-50 p-3 border border-slate-200 flex items-center justify-between">
                <div>
                  <div className="text-xs font-bold text-slate-900">{item.pair}</div>
                  <div className="text-[10px] text-slate-500 font-medium">
                    Actual: <span className="text-indigo-700 font-bold">{item.actual}</span> → Predicted: <span className="text-rose-700 font-bold">{item.predicted}</span>
                  </div>
                </div>
                <div className="text-right">
                  <div className="text-sm font-black text-rose-700">{item.count} errors</div>
                  <div className="text-[10px] text-slate-500 font-semibold">{item.pct_of_errors}% of errors</div>
                </div>
              </div>
            ))}
          </div>
          ...
          <div>
            <div className="border-b border-slate-100 pb-3 flex items-center justify-between">
              <div className="flex items-center space-x-2">
                ...
                <h3 className="text-sm font-bold text-slate-900">PM Action Items for Data Quality</h3>
              </div>
            </div>

            <div className="space-y-3 my-4">
              {errorAnalysis.dataset_action_items.map((item, i) => (
                <div key={i} className="flex items-start space-x-2 rounded-lg bg-indigo-50 p-3 border border-indigo-200 text-xs text-indigo-950 font-medium">
                  <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-indigo-200 font-bold text-indigo-800 text-[10px]">
                    {i + 1}
                  </span>
                  <span>{item}</span>
                </div>
