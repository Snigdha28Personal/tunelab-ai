  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm space-y-4">
          <div className="border-b border-slate-100 pb-3 flex items-center justify-between">
            <span className="text-xs font-bold uppercase tracking-wider text-slate-500">Training Config</span>
            <Layers className="h-4 w-4 text-indigo-600" />
          </div>

          <div className="space-y-3 text-xs">
            <div className="flex justify-between items-center rounded-lg bg-slate-50 p-2.5 border border-slate-100">
              <span className="text-slate-600 font-medium">Training Examples (70% Split)</span>
              <span className="font-bold text-slate-900">{trainCount} items</span>
            </div>
            <div className="flex justify-between items-center rounded-lg bg-slate-50 p-2.5 border border-slate-100">
              <span className="text-slate-600 font-medium">Estimated Tokens / Epoch</span>
              <span className="font-bold text-slate-900">{(trainCount * 150).toLocaleString()} tokens</span>
            </div>
            <div className="flex justify-between items-center rounded-lg bg-slate-50 p-2.5 border border-slate-100">
              <span className="text-slate-600 font-medium">Total Tokens (3 Epochs)</span>
              <span className="font-bold text-indigo-700">{estimatedTokens.toLocaleString()} tokens</span>
            </div>
            <div className="flex justify-between items-center rounded-lg bg-slate-50 p-2.5 border border-slate-100">
              <span className="text-slate-600 font-medium">Estimated Training Cost</span>
              <span className="font-bold text-emerald-700">${((estimatedTokens / 1_000_000) * 3.0).toFixed(4)}</span>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 rounded-xl border border-slate-200 bg-white p-5 shadow-sm space-y-3">
          <div className="border-b border-slate-100 pb-3 flex items-center justify-between">
            ...
            <span className="text-[11px] text-slate-500 font-mono font-semibold">Format: Chat Completions JSONL</span>
          </div>

          <pre className="p-4 rounded-lg bg-slate-900 text-indigo-200 font-mono text-[11px] overflow-x-auto max-h-48 border border-slate-800 shadow-inner">
            {jsonlSnippet}
          </pre>

          <div className="flex items-center space-x-2 text-xs text-emerald-700 bg-emerald-50 p-2.5 rounded-lg border border-emerald-200 font-medium">
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-slate-500 font-medium mt-0.5">
              Launch fine-tuning job on base model <code className="text-indigo-700 font-mono font-bold">gpt-4o-mini</code>.
            </p>
          </div>
          ...
          <button
            ...
            disabled={isTraining}
            className="flex items-center space-x-2 rounded-lg bg-indigo-600 px-5 py-2.5 text-xs font-bold text-white shadow-md shadow-indigo-600/20 hover:bg-indigo-500 disabled:opacity-50"
          >
            {isTraining ? (
              <>
        ...
        {trainingStage && (
          <div className="rounded-lg bg-indigo-50 p-3.5 border border-indigo-200 text-xs font-mono font-semibold text-indigo-900 flex items-center space-x-3">
            {isTraining ? (
